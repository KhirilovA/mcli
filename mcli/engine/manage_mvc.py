...
\r""")
        ...
        for index, item in enumerate(self.links):
            _instances_map += f"{item.pascal_cls_name}Instances.{item.instance_name}:{item.pascal_cls_name}DataItem\n"
            _pascal_names += f"{item.pascal_cls_name}DataItem"
            _m_instances += f"""\ncase {item.pascal_cls_name}Instances.{item.instance_name}:
            ..."""
            ...
            _responses += _response_template.render(name=item.pascal_cls_name)
